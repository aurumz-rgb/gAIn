...
# Try importing required libraries. If not present, the provider options will simply not be available or error gracefully.
try:
...
def get_firebase_stats():
    ...
    try:
        ...
        else: return {"papers_screened": 0, "papers_extracted": 0, "total_visits": 0}
    except Exception as e:
...
class AnthropicProvider(BaseLLMProvider):
    ...
    def generate(self, messages, temperature, max_tokens):
        # Anthropic API expects system prompt separately usually, but we can bundle in messages for simplicity
...
class CohereProvider(BaseLLMProvider):
    def __init__(self, api_key, model_name="command-a-03-2025"): # Updated to requested default
        super().__init__(api_key, model_name)
    ...
    def generate(self, messages, temperature, max_tokens):
        # Cohere chat API
        # Convert openai format messages to cohere format roughly
        chat_history = []
        message_content = ""
        ...
        for msg in messages:
            if msg['role'] == 'user':
                ...
                # Prepend system message to user message for Cohere
...
class GLMProvider(BaseLLMProvider):
    ...
    def generate(self, messages, temperature, max_tokens):
        # ZaiClient expects a single prompt string usually, or similar structure to OpenAI.
        # We will join messages into a single prompt for Zai if it doesn't support history format well,
        # but based on original code it seemed to take raw prompt string.
        # We will assume the latest user message is the prompt.
...
class OllamaProvider(BaseLLMProvider):
    def __init__(self, api_key, model_name="llama3", base_url="http://localhost:11434"):
        # API Key is ignored for local Ollama, but kept for interface consistency
...
def query_llm(prompt, provider_name, api_key, model_name, temperature=0.1, max_tokens=2048):
    """
    ...
    if not api_key and provider_name != "Ollama (Local)":
        # Ollama might not need a key if local auth is off
        return None
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Initialize provider inside retry loop to refresh connections if needed
            ...
            # Generic rate limit detection keywords
...
# Store defaults in session state if not set (Safety check)
# This remains global to ensure variables exist for the "Process" button even if expander isn't opened.
...
if not st.session_state.disclaimer_acknowledged:
    ...
    <ul>
        ...
        <li><strong>No Guarantee of Completeness or Accuracy:</strong> While ReviewAid aims to improve efficiency during literature review and evidence synthesis process, tool does not guarantee the completeness, correctness, or reliability of its results. Users should exercise critical evaluation and cross-check all information before including it in their research.</li>
...
def to_docx(df):
    ...
    Features:
    - Landscape Orientation to accommodate wide data tables (especially for Extractor).
    - Dynamic column widths calculated to fill the page width evenly.
    - Equal width columns ensure vertical consistency across rows ("equal length vertically").
    - Header with Logo (Right).
    - Footer with Text "Generated by ReviewAid on [Latest Date]" (Left).
    - Simple, neat, professional table with white background.
    - Clean font layout (Arial).
    - "Results" heading made bolder.
    - Improved table layout to ensure rows flow continuously and don't force page breaks between every row.
    """
...
def to_pdf(df):
    """
    Generates a PDF with proper table layout.
    Ensures:
    1. Continuous table rows (no one study per page unless page ends).
    2. Columns aligned horizontally.
    3. Footer does not overlap.
    4. Text wraps correctly within cells.
    """
...
if st.session_state.app_mode == "screener":
    ...
    # --- CONFIGURATION EXPANDER (Moved Here) ---
    with st.expander("⚙️ Configuration", expanded=False):
        ...
        # Logic to hide inputs if Default is selected
        if provider_name == "Default":
            st.info("""
                **Default Mode Selected.**
                ...
                """)
            # Store default settings to session state
            st.session_state['provider_name'] = "Default"
            st.session_state['model_name'] = "GLM-4.6V-Flash"
            st.session_state['api_key'] = "HIDDEN" # Placeholder
        
        elif provider_name == "OpenAI":
            model_name = st.text_input("Model Name", value="gpt-4o", help="e.g. gpt-4o, gpt-4-turbo")
            ...
            model_name = st.text_input("Model Name", value="claude-3-5-sonnet-20240620", help="e.g. claude-3-5-sonnet-20240620")
            ...
            # Optional key input for auth-enabled ollama instances
...
if st.button("Process Papers" if st.session_state.app_mode == "extractor" else "Screen Papers"):
    ...
    if selected_provider == "Default":
        ...
        # We map Default to the GLM provider for the actual call
    ...
    try:
        for idx, pdf in enumerate(uploaded_pdfs[:max_papers], 1):
            ...
            # AGGRESSIVE GARBAGE COLLECTION START
            # Force collection before starting next heavy task to ensure RAM is elastic
            gc.collect()
            
            # Explicitly initialize variables to prevent NameError if extraction fails
            matches_exc = []
            matches_inc = []
            result = None
            ...
            try:
                ...
                if pdf_hash in st.session_state.batch_file_hashes:
                    ...
                    # RAM OPTIMIZATION: Delete bytes immediately after use
                    del pdf_bytes 
                    continue
                else:
                    try:
                ...
                # RAM OPTIMIZATION: Delete bytes immediately after extraction
                del pdf_bytes 
                ...
                if st.session_state.app_mode == "screener":
                    ...
                    if len(matches_exc) >= 1 and len(matches_inc) == 0:
                        ...
                        # RAM OPTIMIZATION: Delete variables
                        del text, full_text_backup, matches_exc, matches_inc, title, author, year
                        continue
...
"""
                ...
                # RAM OPTIMIZATION: Delete prompt immediately after LLM call
                # (Note: deletion happens after result is captured)
                ...
                for api_attempt in range(max_api_attempts):
                    ...
                    for retry_idx in range(retries_per_api_attempt):
                        if st.session_state.app_mode == "extractor":
                            ...
                            raw_result = query_llm(prompt, provider_for_call, api_key, model_name, temperature=0.1, max_tokens=2048)
                ...
                # RAM OPTIMIZATION: Delete prompt and large text objects now that we have raw_result
                del prompt 
                del text
                ...
                # RAM OPTIMIZATION: Delete raw result
                if raw_result:
                    del raw_result
                
      
                # RAM OPTIMIZATION: Delete full text backup once used for parsing
                ...
                if result and processing_successful:
                    ...
                    # RAM OPTIMIZATION: Delete metadata vars
                    ...
                    if st.session_state.app_mode == "screener":
                        ...
                        # FIX APPLIED HERE: Increased truncation limit from 1000 to 10000 characters
                ...
                # RAM OPTIMIZATION: Force GC in exception case
                gc.collect()
